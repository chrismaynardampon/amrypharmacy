...
class PriceHistory(APIView):
    def get(self, request, price_history_id=None):
        try:
            ...
            if price_history_id is not None:
                query = query.eq('price_history_id', price_history_id)
    ...
    def put(self, request, price_history_id):
        data = request.data 
        try:
            response = supabase.table("Price_History").update(data).eq('price_history_id', price_history_id).execute()
    ...
    def delete(self, request, price_history_id):
        try:
            response = supabase.table("Price_History").delete().eq('price_history_id', price_history_id).execute()
