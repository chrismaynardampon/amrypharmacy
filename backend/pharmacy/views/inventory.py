...
class Inventory(APIView):
    ...
    def put(self, request, inventory_id):
        data = request.data 
        try:
            response = supabase.table("Inventory").update(data).eq('inventory_id', inventory_id).execute()
    ...
    def delete(self, request, inventory_id):
        try:
            response = supabase.table("Inventory").delete().eq('inventory_id', inventory_id).execute()
