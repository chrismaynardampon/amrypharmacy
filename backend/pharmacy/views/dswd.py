...
class DswdOrder(APIView):
    def get(self, request, dswd_order_id=None):
        try:
            query = supabase.table('Dswd_Order').select('*')
            if dswd_order_id is not None:
                query = query.eq('dswd_order_id', dswd_order_id)
    ...
    def post(self, request):
        ...
        try:
            ...
            response = supabase.table("Dswd_Order").insert(data).execute()
    ...
    def put(self, request, dswd_order_id):
        data = request.data 
        try:
            response = supabase.table("Dswd_Order").update(data).eq('dswd_order_id', dswd_order_id).execute()
    ...
    def delete(self, request, dswd_order_id):
        try:
            response = supabase.table("Dswd_Order").delete().eq('dswd_order_id', dswd_order_id).execute()
